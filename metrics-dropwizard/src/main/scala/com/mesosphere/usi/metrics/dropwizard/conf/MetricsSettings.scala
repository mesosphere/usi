...
case class HistorgramSettings(
  ...
  require(reservoirHighestTrackableValue > 1L,
          "reservoir-highest-trackable-value: should be > 1")
  require(reservoirSignificantDigits >= 0 && reservoirSignificantDigits <= 5,
          "reservoir-significant-digits should be >= 0 and <= 5")
  require(reservoirResettingInterval > Duration.Zero,
          "reservoir-resetting-interval should be > 0")
...
case class StatsdReporterSettings(host: String,
  ...
  require(transmissionInterval > Duration.Zero,
          "transmitiona-interavl should be > 0")
}
  ...
  def fromSubConfig(c: Config): MetricsSettings = apply(
    ...
    dataDogReporterSettings =
      if (c.getBoolean("datadog-reporter"))
        Some(
          DataDogReporterSettings.fromSubConfig(
            c.getConfig("reporters.datadog")))
      else None
  )
}
