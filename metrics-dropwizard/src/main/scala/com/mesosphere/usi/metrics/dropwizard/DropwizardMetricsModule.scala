...
import com.mesosphere.usi.metrics.dropwizard.reporters.{
  DataDogAPIReporter,
  DataDogUDPReporter,
  StatsDReporter
}
...
class DropwizardMetricsModule(config: Config) {
  ...
  def start(actorRefFactory: ActorRefFactory): Done = {
    metricsSettings.statsdReporterSettings.foreach(
      statsdSettings =>
        actorRefFactory.actorOf(StatsDReporter.props(statsdSettings, registry),
                                "StatsDReporter"))

    metricsSettings.dataDogReporterSettings.foreach {
      case s: DataDogUdpReporterSettings =>
        actorRefFactory.actorOf(DataDogUDPReporter.props(s, registry),
                                name = "DataDogUDPReporter")
      case s: DataDogApiReporterSettings =>
        actorRefFactory.actorOf(DataDogAPIReporter.props(s, registry),
                                name = "DataDogAPIReporter")
      case s =>
        throw new IllegalArgumentException(
          s"Unsupported DataDog reporter typ: $s")
    }
    Done
  }
}
