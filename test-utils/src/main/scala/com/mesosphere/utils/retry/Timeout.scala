package com.mesosphere.utils.retry
  ...
  def apply[T](timeout: Duration, name: Option[String] = None)(f: => Future[T])(implicit
    ...
    timeout match {
      case duration: FiniteDuration =>
        def t: Future[T] = after(duration, scheduler)(Future.failed(TimeoutException(s"${name.getOrElse("None")} timed out after ${timeout}")))
