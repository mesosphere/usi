...
import com.mesosphere.usi.core.models.{PodRecordUpdated, SpecEvent, SpecUpdated, SpecsSnapshot, StateEvent, StateSnapshot, StateUpdated}
...
object Scheduler {
  ...
  private[core] def unconnectedGraph(
    ...
    BidiFlow.fromGraph {
      GraphDSL.create(
        new SchedulerLogicGraph(mesosCallFactory),
  ...
  private def persistenceFlow(
    ...
    def persistSchedulerEvents(events: SchedulerEvents): Future[Done] = {
      import CallerThreadExecutionContext.context
      Future.sequence(events.stateEvents.map {
        case _ @ PodRecordUpdated(podId, maybePodRecord) =>
          maybePodRecord match {
            case Some(podRecord) =>
              podRecordRepository.create(podRecord)
            case None =>
    ...
    Flow[SchedulerEvents].mapAsync(1) { events =>
      persistSchedulerEvents(events).map { _ => events }(CallerThreadExecutionContext.context)
    }
  }
