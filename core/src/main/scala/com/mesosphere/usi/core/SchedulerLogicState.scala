...
/**
  ...
  * @param podStatuses
  */
case class SchedulerLogicState(podSpecs: Map[PodId, PodSpec], podRecords: Map[PodId, PodRecord], podStatuses: Map[PodId, PodStatus]) {
  ...
  /**
    ...
    */
  def applyStateIntents(stateIntents: List[StateEvent]): SchedulerLogicState = {
...
object SchedulerLogicState {
  val empty = SchedulerLogicState(Map.empty, Map.empty, Map.empty)
}
