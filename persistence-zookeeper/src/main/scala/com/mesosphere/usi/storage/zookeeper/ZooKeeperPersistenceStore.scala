...
class ZooKeeperPersistenceStore(metrics: Metrics, factory: AsyncCuratorBuilderFactory, parallelism: Int = 10)(
  ...
  /**
    ...
    *
    * Note: Due to current state of the underlying Curator API, [[com.mesosphere.usi.storage.zookeeper.PersistenceStore.CreateOp]]s
  ...
  override def createIfAbsent(node: Node): Future[String] = {
    ...
    create(node).recoverWith {
      case _: NodeExistsException =>
        Future(node.path) // CreateIfAbsent hasn't created a new node since it already exists
    }
  }
}
