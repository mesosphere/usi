...
import com.mesosphere.usi.repository.TemplateRepositoryLike.Template
import com.mesosphere.usi.storage.PathTrie
import com.mesosphere.usi.state.PathId
...
/**
  ...
  * them by calling [[com.mesosphere.usi.storage.zookeeper.ZooKeeperPersistenceStore.children()]] (on the parent znode)
...
class TemplateRepository(val store: ZooKeeperPersistenceStore, val base: String)(implicit val mat: Materializer)
  ...
  /**
    ...
    * @return
    */
  def toNode(template: Template[_]): Node =
    Node(storePath(template.id, version(template)), ByteString(template.toProtoByteArray))
  ...
  def version(bytes: Array[Byte]): String =
  ...
  /**
    * Method does the opposite conversion to the [[toNode]] method by taking in a trie [[com.mesosphere.usi.storage.PathTrie.TrieNode]]
